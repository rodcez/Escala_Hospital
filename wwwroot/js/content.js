...
window.escalaUi = {
    ...
    popularTableBranco: function () {
        ...
        let optionsFuncoesManual = [];
        optionsFuncoesManual.push('<option value="0"></option>');
        window.escalaFunctions.getListaFuncoes().forEach(f => {
            optionsFuncoesManual.push('<option value="' + f.Tipo + '">' + f.Tipo + '</option>');
        });

        for (var x = 0; x < listaDias.length; x++) {
            for (var y = 0; y < listaEnf.length; y++) {
                $("#tr" + listaEnf[y].Id).append('<td> <select class="form-control-sm" id="select-' + x + '-' + y + '"> ' + optionsFuncoesManual.toString().replaceAll(',', '') + ' </select> </td>');
            }
        }
    },
